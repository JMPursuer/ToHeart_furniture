...
service.interceptors.request.use(
  config => {
    ...
    const reg = RegExp('goods/option/upload');
    if (config.url.match(reg)) {
      config.headers['Content-Type'] =
        'multipart/form-data" -F "file=@goods2.xlsx;type=application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"';
    }
    const download = RegExp('goods/option/download');
...
service.interceptors.response.use(
  response => {
    const reg = RegExp('goods/option/download');
    if (response.status === 200 && response.config.url.match(reg)) {
    ...
    if (res.errno === 501) {
      ...
      return Promise.reject('error');
    } else if (res.errno !== 0 && !res.success) {
