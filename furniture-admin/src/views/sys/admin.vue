  ...
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-permission="['POST /admin/user/user/admin/update']"
            type="primary"
    ...
    <!-- 账号权限转移 -->
    <el-dialog title="账号权限转移" :visible.sync="userDialogShow">
      <el-form
        ...
        style="width: 400px; margin-left:50px;"
      >
        <el-form-item label="被转移客户管理员">
          <el-tag>{{ userUpdate.username }}</el-tag>
        </el-form-item>
        <el-form-item label="接收客户管理员" prop="to">
          <el-select
            v-model="userUpdate.to"
            filterable
            remote
            reserve-keyword
            :loading="userLoading"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="updateUserSure">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        to: [
          { required: true, message: '需要选择接收客户管理员', trigger: 'blur' }
        ],
  ...
  methods: {
    remoteMethod(query) {
      if (query !== '') {
        ...
        listAdmin({
          ...
          .then(response => {
            this.toUserList = response.data.data.list;
    ...
    formatRole(roleId) {
      const list =
        this.roleOptions.length > 0
          ? this.roleOptions.filter(res => res.value === roleId)
          : [];
    ...
    updateUserSure() {
      this.$refs['transferDataForm'].validate(valid => {
        if (valid) {
          this.$confirm(
            '此操作将会把该用户下的所属客户全部转移, 是否继续?',
            '提示',
            {
            ...
            userUpdate({
              to: this.userUpdate.to,
              from: this.userUpdate.id
            }).then(res => {
              this.$notify.success({
                title: '成功',
                message: `管理员:${this.userUpdate.username}下的客户已全部转移`
              });
